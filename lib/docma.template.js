...
function _addMetasToDoc($, metaList) {
    ...
    metaList.forEach(metaInfo => {
        const meta = HTMLParser.DOMUtil.N_TAB + HTMLParser.DOMUtil.getMetaElem(metaInfo);
...
class DocmaTemplate {
     ...
     *         Debug instance.
     */
    constructor(config, appConfig, paths, debug) {
        this.$debug = debug;
    ...
    writeServerConfig(routes) {
        ...
        if (this.appConfig.server === 'apache') {
            ...
            return fs.readFile(this.paths.APACHE_CONF, 'utf8')
                .then(content => {
        ...
        if (this.appConfig.server === 'github' || this.appConfig.server === 'static') {
            ...
            return fs.readFile(this.paths.REDIRECT_HTML, 'utf8')
                .then(html => {
            ...
            .then(parser => {
                return parser
                    ...
                    .edit((window, $) => {
                        ...
                        const head = $('head');
                        const domUtil = HTMLParser.DOMUtil;
                        ...
                        // Set title
                        const title = domUtil.N_TAB + '<title>' + this.appConfig.title + '</title>' + domUtil.N_TAB; // eslint-disable-line
                        ...
                        const docmaCss = domUtil.getStyleElem('css/docma.css') + domUtil.N_TAB;
