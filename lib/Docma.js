...
class Docma {
    ...
    _parseJS(jsFiles, filePrefix) {
        ...
        function predicate(symbol) {
            ...
            // if routing = path, no leading slash, or won't work with relative
            // links with app.base
        ...
        return Promise.resolve()
            ...
            .then(() => {
                ...
                if (this.$debug.noMinify || this.$debug.webLogs) {
                    ...
                    msg = msg.join(' and ');
                    const warn = this.$debug.buildLogs ? this.$debug.warn : console.warn;
                    warn(`Before you publish your docs, note that ${msg}.`);
                    const log = this.$debug.buildLogs ? this.$debug.data : console.log;
                    log('Use the \'debug\' option in your build configuration to change this.\n');
                }
                return true;
            });
    }
