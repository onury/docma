...
module.exports = (function () {
    ...
    utils.glob.match2 = function (pattern, options) {
        var g = utils.glob.inspect(pattern, options);
        if (g.negated) return Promise.resolve([]);

        if (!_.isArray(options.ignore)) options.ignore = [];
        ...
        // glob() will not process if the pattern is not a glob but a regular
        // file path.
        if (!g.hasMagic) {
            return Promise.resolve([g.pattern]);
        }
        return glob(g.pattern, options);
    };
