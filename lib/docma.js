            ...
            return utils.glob.match(src)
                .then(function (srcFiles) {
                    _.each(srcFiles, function (filePath) {
                        ...
                        var includePattern = typeof $this.config.jsdoc.includePattern === 'string'
                            ? (new RegExp($this.config.jsdoc.includePattern)).test(filePath)
                            : false;
                ...
                return Promise.reduce(groupNames, function (memo, name) {
                    ...
                    return self._parseJS(group, name)
                        .then(function (data) {
                            ...
                            memo[name] = {
                                documentation: data,
                                symbols: jsdocx.utils.getSymbolNames(data, self.config.jsdoc.sort)
                            };
                            return memo;
                        });
                }, {});
