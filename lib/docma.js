...
// own modules
const DocmaTemplate = require('./DocmaTemplate');
...
class Docma {
    ...
    _init(config) {
        ...
        this.template = new DocmaTemplate(this.config);
    ...
    _compileWebCore() {
        ...
        return fs.ensureDir(jsDest)
            .then(() => {
                this.$debug.data('Compiling components...');
                return Promise.each(Paths.DOCMA_COMPS, file => {
                    this.$debug.data('Compiling:', file);
            ...
            .then(() => this.template.compilePartials())
            .then(dustTemplates => {
                // add dust templates
                ...
                const dm = {
                    ...
                    // options, etc...
                    template: this.template.getData() || {},
            ...
            return fs.readFile(file, 'utf8')
                ...
                .then(html => {
                    ...
                    return parser.edit((window, $) => {
                        ...
                        const mdBookmarks = this.config.markdown.bookmarks;
                        if (mdBookmarks) {
                            const selector = typeof mdBookmarks === 'string'
                                ? mdBookmarks
                                : ':header'; // all: "h1, h2, h3, h4, h5, h6"
                            $(selector).each(function () { // no arrow here!
                                const heading = $(this);
            ...
            return utils.glob.match(copyList)
                .then(files => {
                    return Promise.each(files, filePath => {
                        ...
                        this.$debug.data('Copying:', fPath, '»', dest);
    ...
    build(config) {
        return Promise.resolve()
            ...
            .then(conf => {
                ...
                this.$debug.log('Using Template:', this.template.config.name, `(version: ${this.template.config.version})`);

                if (!this.template.checkVersion()) {
                    const vmsg = `Template requires Docma version ${this.template.config.docmaVersion}`;
            ...
            .then(() => {
                ...
                return Promise.reduce(groupNames, (memo, name) => {
                    ...
                    return this._parseJS(group, name)
                        .then(data => {
                            this._addRoute(name, true); // e.g. name=docma-web —> api:docma-web
                            memo[name] = {
                                documentation: data,
            ...
            .then(() => this.template.copyToDest())
            .then(() => this.template.compileStyles())
            .then(() => this.template.compileScripts())
            .then(() => this.template.writeMainHTML())
            .then(() => this._parseMD(this.files.md))
            .then(() => this._processHTML(this.files.html))
            .then(() => this._copyAssets())
            .then(() => this._compileWebCore())
            .then(() => {
                if (this.config.app.routing.method === 'path') {
                    return this.template.writeServerConfig(this.routes);
                }
            })
            .then(() => this.template.postProcess())
            .then(() => {
