...
(function () {
     ...
     *  with signature `{ documentation:Array, symbols:Array }`. If omitted,
     *  current documentation array is used.
     *
     *  @returns {Object} - Symbol object if found. Otherwise, returns `null`.
     */
    utils.getSymbolByName = function (name, docsOrApis) {
        ...
        if (utils.type(docsOrApis) === 'object') {
            ...
            for (i = 0; i < apiNames.length; i++) {
                ...
                found = utils.getSymbolByName(name, docs);
                if (found) return found;
            }
            return null;
        }

        docs = docsOrApis || docma.documentation; // current docs if omitted
        for (i = 0; i < docs.length; i++) {
            ...
            if (symbol.$members) {
                found = utils.getSymbolByName(name, symbol.$members);
                if (found) return found;
            }
        }
        return null;
     ...
     */
    utils.getSymbolLink = function (symbolOrName) {
        if (typeof symbolOrName !== 'string') {
            return symbolOrName.$docmaLink;
        }
        var symbol = utils.getSymbolByName(symbolOrName, docma.apis);
     ...
     *  @returns {String}
     */
    utils._getAnchorLink = function (type, options) {
        var opts = options || {};
        var link;
        var target = '';
        ...
        if (!link && opts.linkType !== 'external') link = utils.getSymbolLink(type);
     ...
     */
    utils.getTypes = function (symbol, options) {
        ...
        types = types.map(function (type) {
            ...
            type = type.replace(/\.</g, '<');
            if (opts.links) type = utils._getAnchorLink(type, { linkType: opts.links });
            return type;
        }).join('|');
     ...
     *  @returns {String}
     */
    utils.getReturnTypes = function (symbol, options) {
            ...
            if (Array.isArray(names)) {
                if (opts.links) {
                    names = names.map(function (name) {
                        return utils._getAnchorLink(name, { linkType: opts.links });
                    });
                }
     ...
     *  @returns {String}
     */
    utils.getCodeTags = function (list, options) {
        var opts = options || {};
        var escaped;
        return list.map(function (item) {
            escaped = utils._wrapEscapeCode(item);
            if (opts.links) {
                escaped = utils._getAnchorLink(item, {
                    displayText: escaped,
                    linkType: opts.links
                });
            }
    ...
    utils.getFormattedTypeList = function (list, options) {
        ...
        var pList = list.map(function (item) {
            ...
            types = types.map(function (type) {
                ...
                if (opts.links) {
                    escaped = utils._getAnchorLink(type, {
                        linkType: opts.links,
                        displayText: escaped
                    });
                }
     ...
     *  @returns {String}
     */
    utils.getEmittedEvents = function (list, options) {
        if (!list || list.length === 0) return '';
        ...
        var events = (list || []).map(function (event) {
            ...
            if (opts.links) {
                escaped = utils._getAnchorLink(name, {
                    linkType: opts.links,
                    displayText: escaped
                });
            }
