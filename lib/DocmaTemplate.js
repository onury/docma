...
class DocmaTemplate {
    ...
    _promiseTemplateProcess(fn, args = [], options = {}) {
        ...
        if (!this.module || typeof this.module[fn] !== 'function') {
            return Promise.resolve(opts.defaultResolveValue);
        }
        if (typeof opts.log === 'function') opts.log();
        // ensure promise
        return Promise.resolve(this.module[fn](...args))
            .then(value => {
    ...
    preCompile(rawContent, src, dest) {
        return this._promiseTemplateProcess(
            ...
            [rawContent, src, dest],
            {
                ...
                log: () => {
                    this.$debug.data(`Running template pre-compile process for ${path.relative(this.module.dirname, src)}`);
                }
            }
        );
    }
    ...
    postCompile(compiledContent, src, dest) {
        return this._promiseTemplateProcess(
            ...
            [compiledContent, src, dest],
            {
                ...
                log: () => {
                    this.$debug.data(`Running template post-compile process for ${path.relative(this.module.dirname, src)}`);
                }
            }
        );
    }
